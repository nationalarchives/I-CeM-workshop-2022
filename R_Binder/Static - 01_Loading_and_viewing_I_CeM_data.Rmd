...
fieldname = 'Occ' # Other options ["Censusref", "PID", "Occ", "Sex", "Parish", "Cond", "Relat", "Age"]
aggregation_function = 'Value' # Choose from ["Value","ValueFormat","ShortFormat"]
"records" = 10
...
mutate(aggregate_column = do.call(rbind,lapply(aggregate_column, function_to_use)))  %>%
...
```{r}
#@title Select a field to summarise { run: "auto", vertical-output: true }
FieldName = 'Parish' # Other options ["Censusref", "PID", "Occ", "Sex", "Parish", "Cond", "Relat", #"Age"]
PartitionField = 'is_child' # Choose from ["BlankCol", "Sex", "is_child"]
records = 15


options(fig.width=20, fig.height=8, warn = -1)
